...
function logRetrieveMethods(retrieveMethods) {
  ...
  retrieveMethods.forEach(retrieveMethod =>
    console.log(`  ${retrieveMethod.toString()}`)
  );
  console.log("\n");
}
...
Will run ${times.toLocaleString()} times for each test case.
  ...
  testCases.forEach(testCase => {
    if (typeof testCase === "object") {
      ...
      const { duration } = testMultipleTimes(
        () => testType(value, retrieveMethod),
        times
      );
      console.log(
        "    ",
        retrieveMethod.name.padEnd(20, " "),
        "took",
        duration.toLocaleString().padStart(5, " "),
        "ms"
      );
    } else {
...
function impossibleRetrieve(_) {
  return _[Math.random()][Math.random()];
}
function shallowRetrieve(_) {
  return _.key;
}
function deepRetrieve(_) {
  return _.a.b.c.d.e.f.g.h.i;
}
function deepChainedRetrieve(_) {
  return (
    _.a &&
    _.a.b &&
    _.a.b.c &&
...
test(
  ...
  [protectByTryCatch, protectBySafeTouch],
  [impossibleRetrieve, shallowRetrieve, deepRetrieve],
  [
    {
...
test(
  ...
  [noProtections, protectBySafeTouch],
  [deepRetrieve, deepChainedRetrieve],
  [
    {
      name: "Deep Object",
      value: { a: { b: { c: { d: { e: { f: { g: { h: { i: true } } } } } } } } }
